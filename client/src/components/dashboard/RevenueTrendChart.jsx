...
export const RevenueTrendChart = () => {
  ...
  return (
    <div className="p-6 rounded-3xl bg-white border border-[#E7E0F7] shadow-soft-sm flex flex-col justify-between space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-black text-slateText-main">Weekly Sales & Revenue Trend</h3>
            <span className="px-2 py-0.5 rounded-full bg-emeraldGreen-100 text-emeraldGreen-800 text-[10px] font-black uppercase">
              +24.8% This Week
            </span>
          </div>
          ...
          {weeklyData.map((item, idx) => (
            <div key={idx} className="flex-1 flex flex-col items-center gap-2 group h-full justify-end">
              ...
              <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-200 text-center pointer-events-none mb-1">
                <span className="bg-slate-900 text-white text-[10px] font-black px-2 py-1 rounded-lg shadow-lg whitespace-nowrap">
                  {formatCurrency(item.revenue)} ({item.orders} orders)
                </span>
              </div>

              {/* Bar Column */}
              <div className="w-full max-w-[48px] bg-slate-100 rounded-2xl overflow-hidden flex flex-col justify-end h-36 p-1">
                <div
              ...
              {/* Day Label */}
              <span className={`text-[11px] font-bold ${item.isPeak ? 'text-[#6C4DF6] font-black' : 'text-slate-500'}`}>
                {item.day}
              </span>
            </div>
          ))}
      ...
      <div className="pt-4 border-t border-slate-100 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-emerald-50 border border-emerald-200/60 text-xs font-bold text-emerald-700">
            <Smartphone className="w-3.5 h-3.5" />
            <span>Instant UPI (68%) • ₹1.93L</span>
          </div>

          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-blue-50 border border-blue-200/60 text-xs font-bold text-blue-700">
            <CreditCard className="w-3.5 h-3.5" />
            <span>Cards / Netbanking (22%) • ₹62.6K</span>
          </div>

          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-amber-50 border border-amber-200/60 text-xs font-bold text-amber-700">
            <Truck className="w-3.5 h-3.5" />
            <span>Cash on Delivery (10%) • ₹28.5K</span>
          </div>
        </div>
      </div>
    </div>
